...
class Nodo:
    def __init__(self, estado, padre, movimiento, profundidad, piezas_correctas):
        ...
        # Movimiento para encontrar este nodo desde el padre.
        self.movimiento = movimiento
        # Posición del nodo en el árbol de búsqueda.
        self.profundidad = profundidad
        # Total de piezas en su lugar para este estado.
    ...
    def encontrar_sucesores(self):
        ...
        sucesores.append(Nodo(sucesorN, self, "arriba",
                         self.profundidad + 1, calcular_heurisitica(sucesorN)))
        sucesores.append(Nodo(sucesorS, self, "abajo",
                         self.profundidad + 1, calcular_heurisitica(sucesorS)))
        sucesores.append(Nodo(sucesorE, self, "derecha",
                         self.profundidad + 1, calcular_heurisitica(sucesorE)))
        sucesores.append(Nodo(sucesorO, self, "izquierda",
                         self.profundidad + 1, calcular_heurisitica(sucesorO)))
...
def bfs(inicial, meta):
    # Conjunto de estados visitados para no visitar el mismo estado más de una vez.
    visitados = set()
    # Cola de nodos aún por explorar. Se agrega el nodo inicial.
    frontera = deque()
    frontera.append(Nodo(inicial, None, None, 0,
                    calcular_heurisitica(inicial)))
    ...
    while frontera:  # Mientras haya nodos por explorar:
        ...
        if nodo.estado == meta:  # Si es una meta,
            print("\n¡Se encontró la meta!")
            # se regresa el camino para llegar a él y termina el algoritmo.
            return nodo.encontrar_camino(inicial)
        else:  # Si no es una meta,
            # se agregan sus sucesores a los nodos por explorar.
...
def dfs(inicial, meta, profundidad_max):
    # Conjunto de estados visitados para no visitar el mismo estado más de una vez.
    visitados = set()
    # Pila de nodos aún por explorar. Se agrega el nodo inicial.
    frontera = deque()
    frontera.append(Nodo(inicial, None, None, 0,
                    calcular_heurisitica(inicial)))
    ...
    while frontera:  # Mientras haya nodos por explorar:
        ...
        # Si es una meta, se regresa el camino para llegar a él y termina el algoritmo.
        if nodo.estado == meta:
            print("\n¡Se encontró la meta!")
            return nodo.encontrar_camino(inicial)
        else:  # Si no es una meta:
            if profundidad_max > 0:  # Si se estableció una búsqueda con profundidad limitada
                if nodo.profundidad < profundidad_max:  # y no se ha llegado al límite,
                    # se agregan los sucesores a los nodos por explorar.
                    frontera.extend(nodo.encontrar_sucesores())
...
def hc(inicial):
    ...
    # Mientras el estado actual no tenga todas las piezas en su lugar:
    while nodo_actual.piezas_correctas < 9:
        # Se buscan los sucesores del estado actual
...
def main():
    ...
    (Nodo(estado_inicial, None, None, 0,
     calcular_heurisitica(estado_inicial))).imprimir_nodo()
